...
const App = () => {
  ...
  const renderItem = ({ item }: { item: Product }) => {
    ...
    const handleIncreaseAmount = async (itemId: number) => {
        try {
          const response = await axios.put(`http://lars.detestbaas.nl:3000/products/${itemId}`, {
            ...item,
            Amount: item.Amount + 1 // Update the amount as needed
          });
          // Update the local state with the updated data from the response
          setProducts(prevProducts =>
      ...
      const handleDecreaseAmount = async (itemId: number) => {
        try {
          const response = await axios.put(`http://lars.detestbaas.nl:3000/products/${itemId}`, {
            ...item,
            Amount: item.Amount - 1 // Update the amount as needed
